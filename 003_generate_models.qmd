...
const_contig <- cartogram_cont(df,weight="population") |> 
  mutate(degree_educated = df$degree_educated,
         health_not_good = df$health_not_good,
         white = df$white,
         younger_adults = df$younger_adults,
...
carto_reg <- const_contig |> 
  group_by(region) |> 
  summarise()

int1 <- ggplot() + 
  geom_sf(data=const_contig,aes(fill=degree_educated), linewidth=0) + 
  geom_sf(data=carto_reg, fill=NA, linewidth=0.3, colour="black") + 
  coord_sf(datum = NA) + 
...
int2 <- ggplot() + 
  geom_sf(data=const_contig,aes(fill=health_not_good), linewidth=0) + 
  geom_sf(data=carto_reg, fill=NA, linewidth=0.3, colour="black") + 
  coord_sf(datum = NA) + 
  scale_fill_distiller(palette = "BrBG", direction = 1) + 
  labs(fill="health\nnot good") + 
...
int3 <- ggplot() + 
  geom_sf(data=const_contig,aes(fill=white), linewidth=0) + 
  geom_sf(data=carto_reg, fill=NA, linewidth=0.3, colour="black") + 
  coord_sf(datum = NA) + 
  scale_fill_distiller(palette = "BrBG", direction = 1) + 
  labs(fill="white") + 
...
int4 <- ggplot() + 
  geom_sf(data=const_contig,aes(fill=younger_adults), linewidth=0) + 
  geom_sf(data=carto_reg, fill=NA, linewidth=0.3, colour="black") + 
  coord_sf(datum = NA) + 
  scale_fill_distiller(palette = "BrBG", direction = 1) + 
  labs(fill="younger\nadults") + 
...
group1 <- ggarrange(int1,int2,int3,int4, nrow = 2, ncol = 2)

group2 <- ggplot() + 
  geom_sf(data=const_contig,aes(fill=con_swing), linewidth=0) + 
  geom_sf(data=carto_reg, fill=NA, linewidth=0.5, colour="black") + 
  coord_sf(datum = NA) + 
  scale_fill_gradient2(low="darkred",mid="white",high="darkblue",midpoint = 0) + 
  labs(fill="conservative swing") + 
  theme_bw() + 
...
ggarrange(group2,group1, ncol = 2, widths = c(1,1.5))
...
kbl(models_table5, caption="Summary of region model outputs") |> 
  kable_classic(full_width = F, html_font = "Cambria") |> 
  row_spec(3:5, background = "#F5E4E6")
...
dorl_mod_mixed_lmer <- ggplot(regions_dorling) + 
  ...
  labs(fill="Random\neffects",
       title = "Model: mod_mixed_lmer",
       subtitle = "Random effects by region") + 
  theme_bw() + 
  theme(legend.position = "bottom") + 
...
# map polygons with equal numbers of voters per pixel
regions_dorling2 <- cartogram_dorling(region_pop,weight="pop") |> 
  left_join(mod_mixed_lmer_rand, by=c("region"="level")) |> 
  mutate(signifigant = case_when(conf.low <0 & conf.high<0 ~ "yes",
                                  conf.low >0 & conf.high>0 ~ "yes",
                                  TRUE ~ "no"),
         reg_abbrev = case_when(region == "Wales" ~ "W",
...
dorl_mod_mixed_lmer_p2 <- ggplot() + 
  geom_sf(data=regions_dorling2,aes(fill=estimate,colour=signifigant), linewidth=0.5) + 
  geom_sf_text(data=regions_dorling2,aes(label=reg_abbrev)) + 
  scale_fill_gradient2(low="darkred",mid="white",high="darkblue",midpoint=0) + 
...
mod_mixed_lmer_correl@call

mod_mixed_lmer_correl_fix <- broom.mixed::tidy(mod_mixed_lmer_correl, effects="fixed", conf.int=TRUE, conf.method = "profile")

#like_rat_pval <- as.data.frame(anova(mm2.2,mm2.1))[2,8]
mod_mixed_lmer_correl_fix$swingto <- case_when(mod_mixed_lmer_correl_fix$estimate>0 ~ "Conservative",
                              TRUE ~ "Labour")
ggplot(mod_mixed_lmer_correl_fix,aes(x=estimate, y=term,xmin=conf.low,xmax=conf.high,fill=swingto,colour=swingto)) + 
  geom_point(shape=21, size=3) + 
  geom_errorbar() + 
    scale_fill_manual(breaks = c("Conservative", "Labour"),
                      values = c("darkblue", "firebrick4")) + 
    ...
    geom_vline(xintercept = 0, colour="red") + 
    theme_bw() + 
  labs(title = "Model: mod_mixed_lmer_correl",
       subtitle = "Fixed effects") + 
  xlim(-4,6)
...
mod_mixed_lmer_correl_rand <- broom.mixed::tidy(mod_mixed_lmer_correl, effects="ran_vals", conf.int=TRUE)

mod_mixed_lmer_correl_plot_list <- list()
i=1
for (var in levels(df_scaled$region)){
  mmtemp <- mod_mixed_lmer_correl_rand[mod_mixed_lmer_correl_rand$level==levels(df_scaled$region)[i],]
  mmtemp$swingto <- case_when(mmtemp$estimate>0 ~ "Conservative",
                              TRUE ~ "Labour")
  mod_mixed_lmer_correl_plot_list[[i]] <- ggplot(mmtemp, aes(x=estimate, y=term,
                                              xmin=conf.low,
                                              xmax=conf.high, 
                                              fill=swingto,
                                              colour=swingto)) + 
...
(plot_spacer()|plot_spacer()|mod_mixed_lmer_correl_plot_list[[5]]|plot_spacer()) / 
  (plot_spacer()|mod_mixed_lmer_correl_plot_list[[4]]|mod_mixed_lmer_correl_plot_list[[6]]|mod_mixed_lmer_correl_plot_list[[11]]|plot_spacer()) / 
  (mod_mixed_lmer_correl_plot_list[[9]]|mod_mixed_lmer_correl_plot_list[[10]]|mod_mixed_lmer_correl_plot_list[[2]]|mod_mixed_lmer_correl_plot_list[[1]]) /
  (plot_spacer()|mod_mixed_lmer_correl_plot_list[[8]]|mod_mixed_lmer_correl_plot_list[[3]]|mod_mixed_lmer_correl_plot_list[[7]]) +
  plot_annotation(
    title = "Model: mod_mixed_lmer_correl",
    subtitle = "Random intercepts and coefficients",
    theme = theme(plot.title = element_text(size = 16))
  )
...
# map polygons with equal numbers of voters per pixel
regions_dorling <- cartogram_dorling(region_pop,weight="pop") |> 
  left_join(mod_mixed_lmer_correl_rand, by=c("region"="level")) |> 
  mutate(sig_estimate = case_when(conf.low <0 & conf.high<0 ~ estimate,
                                  conf.low >0 & conf.high>0 ~ estimate,
                                  TRUE ~ 0))

dorl_mod_mixed_lmer_correl <- ggplot(regions_dorling) + 
  geom_sf(aes(fill=sig_estimate)) + 
  scale_fill_gradient2(low="darkred",mid="white",high="darkblue",midpoint = 0) + 
  coord_sf(datum=NA) + 
  labs(fill="Random\neffects",
...
dorl_mod_mixed_lmer_correl_p2 <- ggplot() + 
  geom_sf(data=regions_dorling2,aes(fill=estimate,colour=signifigant), linewidth=0.5) + 
  geom_sf_text(data=regions_dorling2,aes(label=reg_abbrev)) + 
  scale_fill_gradient2(low="darkred",mid="white",high="darkblue",midpoint=0) + 
  scale_colour_manual(values = c("gray90","black")) + 
...
temp_mod_mixed_lmer_correl <- df |> 
  select(con_swing,constituency_name,constituency_name) |> 
  mutate(resids = resid(mod_mixed_lmer_correl))

df_nb <- poly2nb(df)
df_wts <- nb2listw(df_nb, zero.policy = TRUE)

mp <- moran.plot(temp_mod_mixed_lmer_correl$resids, df_wts, zero.policy = TRUE, labels = temp_mod_mixed_lmer_correl$constituency_name, plot=FALSE) |> 
  mutate(region=df$region)
...
ggplot(mp, aes(x=x, y=wx)) + geom_point(shape=1) + 
  ...
  ylab(paste0("Spatially lagged Residuals")) +
  scale_fill_brewer(palette = "Set3") + 
  labs(title = "mod_mixed_lmer_correl",
       subtitle = paste0("Slope = ",round(temp_reg$coefficients[2],2))) + 
  theme(legend.position = "bottom")
...
dorl_mod_mixed_gam_v1_p2 <- ggplot() + 
  geom_sf(data=regions_dorling2,aes(fill=value,colour=signifigant), linewidth=0.5) + 
  geom_sf_text(data=regions_dorling2,aes(label=reg_abbrev)) + 
  scale_fill_gradient2(low="darkred",mid="white",high="darkblue",midpoint=0) + 
  scale_colour_manual(values = c("gray90","black")) + 
  coord_sf(datum=NA) + 
...
ggplot(mod_mixed_gam_v2_fixed,aes(x=value, y=term,xmin=lower_2.5,xmax=upper_97.5,fill=swingto,colour=swingto)) + 
  ...
  labs(title = "Model: mod_mixed_gam_v2",
       subtitle = "Main effect coefficients") + 
  xlim(-4,6)
...
temp3$variable <- str_remove(temp3$variable, "region..")
temp3$variable <- str_replace(temp3$variable, "region.", "(Intercept)")
...
(plot_spacer()|plot_spacer()|mod_mixed_gam_v2_plot_list[[5]]|plot_spacer()) / 
  ...
  plot_annotation(
    ...
    subtitle = "Interaction of intercepts/coefficients with regions, smoothed by MRF",
    theme = theme(plot.title = element_text(size = 16))
  )
...
dorl_mod_mixed_gam_v2_p2 <- ggplot() + 
  geom_sf(data=regions_dorling2,aes(fill=effect,colour=signifigant,label=region), linewidth=0.5) + 
  geom_sf_text(data=regions_dorling2,aes(label=reg_abbrev)) + 
  scale_fill_gradient2(low="darkred",mid="white",high="darkblue",midpoint=0) + 
  scale_colour_manual(values = c("gray90","black")) + 
  coord_sf(datum=NA) + 
...
lmer_lrt_df <- anova(mod_mixed_lmer,mod_mixed_lmer_correl) |> 
  mutate(model = c("mod_mixed_lmer","mod_mixed_lmer_correl")) |> 
  select(model, everything())

kbl(lmer_lrt_df, caption="LMER/LMERcorrel models: Likelihood ratio test p-values") |> 
  kable_classic(full_width = F, html_font = "Cambria")
...
## Together: Spatial effects on coefficients
...
p_fixed_lmer <- ggplot(mod_mixed_lmer_fix,aes(x=estimate, y=term,xmin=conf.low,xmax=conf.high,fill=swingto,colour=swingto)) + 
  geom_point(shape=21, size=3) + 
  geom_errorbar() + 
    scale_fill_manual(breaks = c("Conservative", "Labour"),
                      values = c("darkblue", "firebrick4")) + 
...
p_fixed_lmer_corr <- ggplot(mod_mixed_lmer_correl_fix,aes(x=estimate, y=term,xmin=conf.low,xmax=conf.high,fill=swingto,colour=swingto)) + 
  geom_point(shape=21, size=3) + 
  geom_errorbar() + 
    scale_fill_manual(breaks = c("Conservative", "Labour"),
                      values = c("darkblue", "firebrick4")) + 
...
p_fixed_gam_v1 <- ggplot(mod_mixed_gam_v1_fixed,aes(x=value, y=term,xmin=lower_2.5,xmax=upper_97.5,fill=swingto,colour=swingto)) + 
  geom_point(shape=21, size=3) + 
  geom_errorbar() + 
    scale_fill_manual(breaks = c("Conservative", "Labour"),
                      values = c("darkblue", "firebrick4")) + 
...
pgeo1_fixed_gam_v1 <- ggplot(mod_mixed_gam_v1_geog,
                                     aes(x=effect, y=region,xmin=conf_lower,xmax=conf_upper,fill=swingto,colour=swingto)) + 
  geom_point(shape=21, size=3) + 
  geom_errorbar() + 
  scale_fill_manual(breaks = c("Conservative", "Labour"),
  ...
  theme_bw() + 
  labs(title = "Model: mod_mixed_gam_v1",
       subtitle = "Varying intercept smoothed by MRF") + 
  xlim(-4,2) + 
  theme(legend.position = "bottom")

pgeo2_fixed_gam_v1 <- ggplot(regions_contig) + 
  geom_sf(aes(fill=fit.s.region.), lwd=0.1) + 
  scale_fill_gradient2(low="darkred",mid="white",high="darkblue",midpoint = 0) + 
  coord_sf(datum=NA) + 
  labs(fill="Geographic\neffect",
...
p_fixed_gam_v2 <- ggplot(mod_mixed_gam_v2_fixed,aes(x=value, y=term,xmin=lower_2.5,xmax=upper_97.5,fill=swingto,colour=swingto)) + 
  geom_point(shape=21, size=3) + 
  geom_errorbar() + 
  scale_fill_manual(breaks = c("Conservative", "Labour"),
                    values = c("darkblue", "firebrick4")) + 
...
## Output of four models of interest
...
ggarrange(p_fixed_lmer,dorl_mod_mixed_lmer_p2, ncol = 2, widths = c(1,1.3))
...
ggarrange(p_fixed_lmer_corr,dorl_mod_mixed_lmer_correl_p2, ncol = 2, widths = c(1,1.3))
...
ggarrange(p_fixed_gam_v1,dorl_mod_mixed_gam_v1_p2, pgeo1_fixed_gam_v1, pgeo2_fixed_gam_v1, ncol = 2, nrow=2, widths = c(1,1.3))
...
ggarrange(p_fixed_gam_v2,dorl_mod_mixed_gam_v2_p2, ncol = 2, widths = c(1,1.3))
...
temp_pie_region <- df |> group_by(region) |> 
  summarise(con_19=sum(con_vote_19, na.rm = TRUE),
            lab_19=sum(lab_vote_19, na.rm = TRUE),
            ld_19=sum(ld_vote_19, na.rm = TRUE),
            lab_17=sum(lab_vote_17, na.rm = TRUE),
            pc_19=sum(pc_vote_19, na.rm = TRUE),
            con_17=sum(con_vote_17, na.rm = TRUE),
            con_swing=sum(con_swing)) |> 
  mutate(con_change = con_19-con_17)

temp_pie_region1 <- temp_pie_region |> 
  mutate(lab_up = ifelse(con_change <0, abs(con_change), NA),
         con_up = ifelse(con_change >0, con_change, NA))

temp_pie_region2<-st_coordinates(st_centroid(temp_pie_region1))
temp_pie_region3<-cbind(temp_pie_region1,temp_pie_region2)

region_pieglyph <- ggplot() + 
  geom_sf(data=regions, fill="gray99", colour="black", linewidth=1) + 
  geom_sf(data=temp_pie_region3,fill=NA) + 
  geom_pie_glyph(data=temp_pie_region3,aes(x=X,y=Y),slices=c("con_17","con_up","lab_up","lab_19","ld_19","pc_19"),
                 radius=1.1,colour="black") +
  scale_fill_manual(values = c("#A1B1F9","darkblue","darkred","#F9A1A4","yellow","darkgreen")) + 
  geom_sf_label_repel(data=temp_pie_region3,aes(label=region),size=2.8, alpha=0.8) + 
  coord_sf(datum=NA) + 
  labs(fill = "Voteshare",
       title = "Regions") + 
  theme_bw() + 
...
centroids <- df |> st_centroid() |> st_coordinates() |> data.frame()
df_arrows <- df |> 
  cbind(centroids) |> 
  mutate(direction = case_when(con_swing > 0 ~ "to Conservative",
                               TRUE ~ "to Labour"))
ggplot() + 
  geom_segment(data=df_arrows, 
               aes(x=X,
                   y=Y, 
                   xend = X + 2700*(con_swing*0.8), 
                   yend = Y + 2700*(abs(con_swing)*0.8), 
                   colour=direction,
                   size=direction), 
               lineend = "round", linejoin = "round",
               arrow = arrow(length = unit(0.15,"cm"))) + 
  geom_sf(data=df, fill=NA) + 
...
df$outcome19 <- case_when(df$winner_19 == "Labour" ~ "Labour hold",
                          df$winner_19 == "Conservative" ~ "Conservative hold",
                          TRUE ~ "Other")
df$outcome192 <- case_when(df$winner_19 == "Conservative" & df$winner_17 == "Labour" ~ "Conservative flip",
                          TRUE ~ df$outcome19)
ggplot(df) + 
  geom_histogram(aes(x=con_swing,fill=outcome192), binwidth = 0.25) + 
  scale_fill_manual(values = c("orange","darkblue","darkred","grey")) + 
  labs(fill = "Seat status") + 
  geom_vline(xintercept = 0) + 
  geom_vline(xintercept = 10, colour="darkgreen") + 
  geom_vline(xintercept = mean(df$con_swing), linetype="dashed", colour="pink") + 
