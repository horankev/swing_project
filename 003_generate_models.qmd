...
models_table2 <- models_table |> 
  mutate(framework = case_when(package == "gam" ~ "gam",
                                  TRUE ~ "other"))

models_table3 <- models_table2 |> filter(package == "gam") |> 
  mutate(rel_AIC = `AIC (min to max)` - models_table[models_table$model=="mod_mixed_gam",]$`AIC (min to max)`)

models_table4 <- models_table2 |> filter(package != "gam")  |> 
...
ggarrange(group2,group1, ncol = 2, widths = c(1,1.3))
